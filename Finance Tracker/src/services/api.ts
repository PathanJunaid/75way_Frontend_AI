...
export const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    predict: builder.mutation<ApiResponse<SpendingData>, { months: number }>({
      query: (body) => {
        return {url: `/predict/${body.months}`, method: 'GET'}
      }
    })
  }),
});

export const { useMeQuery, useLoginMutation, useLogoutMutation, useRegisterMutation, useUpdateUserMutation, usePredictMutation } = api;
